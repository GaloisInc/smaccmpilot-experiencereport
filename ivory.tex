...
using a quasiquoter interface.  Struct \cd{foo\_struct} contains two fields
...
memory-area type.  The possible scope types denote local, or stack-allocated
scope, or global and statically allocated scope.  In the example, the parameter
to \cd{foo\_init} can be a reference with either scope.  The memory area pointed
to be the reference has the type of the struct defined above.
...
monad, returning unit.  The Ivory monad also contains an effects parameter that
tracks effects, as described above, using a type-level record.

Two statements implement the program, both of which update memory areas.  The
\cd{store} operator takes a reference and a value, and updates the memory area
pointed to by the reference with the value.  The struct accessor operator,
...
\end{code}
\noindent
again eliding type class and data-kind constraints.  Note, however, that \cd{\~>}
returns a reference to the field, not the field value.

The \cd{bar} field of the struct pointed to us updated with 1.  Updating the
array in \cd{baz} is slightly more complicated since it contains an array. In
...
number.  An index type \cd{Ix n} only supports indexes from \cd{n} to \cd{n-1}.
