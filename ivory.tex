...
define the Ivory type \cd{Struct "foo\_struct"}. The code generation includes
the well-typed accessor functions: in this example, the functions \cd{bar ::
Label "foo\_struct" (Stored Uint8)} and \cd{baz :: Label "foo\_struct" (Array 10
(Stored Sint16)} are defined for selecting the appropriate fields in any
reference to a \cd{Struct "foo\_struct"}.
...
foo\_init = proc "foo\_init" $ body $ do
  store  (fg\_ref \mytilde{}> bar)      1
  store ((fg\_ref \mytilde{}> baz) ! 0) 2
  where
