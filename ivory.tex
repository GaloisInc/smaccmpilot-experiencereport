...
\cd{"fooStruct"}, that is unique to the defined struct.  The \cd{Stored} type
constructor signifies that the value is allocated in-memory~\cite{memareas}.
The \cd{Array} type constructor takes a type-level natural number as a parameter
...
system since array lengths are statically known.  Ivory exposes an \cd{arrayMap}
...
The operation (\cd{ref \mytilde> baz}) takes the reference to a struct and
...
additional features to prevent unsafe programs.  For example, if \cd{setBaz}
